...
print()
print(f"  成交量趋势斜率: {((mock_kline[-1]['volume'] - mock_kline[0]['volume']) / mock_kline[0]['volume']:.2%}")
print(f"  价格变化: {(mock_kline[-1]['close'] - mock_kline[0]['close']) / mock_kline[0]['close']:.2%}")
print()
...
print(f"    tail_vol_struct = {tail_vol_score:.4f} (偏离0.5 = {tail_vol_score-0.5:+.4f})")
print(f"    增强贡献 = {tail_vol_weight:.2f} × {tail_vol_score-0.5:+.4f} = {(tail_vol_weight * (tail_vol_score - 0.5)):+.4f}")
print(f"    M57增强 = {m57_base:.2f} + {(tail_vol_weight * (tail_vol_score - 0.5)):+.4f} = {m57_enhanced:.4f}")
print()
