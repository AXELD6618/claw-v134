...
V13.4.3 Updates:
  - Added retry logic to ALL steps (T0-T5, NIGHT, BATTLE)
  - Integrated Watchdog monitoring for consecutive failure tracking
  - Auto-alerting via PushPlus when threshold exceeded
"""
...
# Import watchdog (optional — won't break pipeline if missing)
try:
    from cloud_watchdog import record_success, record_failure, run_watchdog_check
    WATCHDOG_AVAILABLE = True
    print("[WATCHDOG] ✅ Watchdog module loaded")
except Exception as e:
    WATCHDOG_AVAILABLE = False
    print(f"[WATCHDOG] ⚠️ Watchdog module not available: {e}")
...
def run_step_T0():
    ...
    for attempt in range(MAX_RETRY + 1):
        try:
            ...
            print(f"[T0] ✅ Success: {result['total_stocks']} stocks, {result['candidates']} candidates")
            # Watchdog: record success
            if WATCHDOG_AVAILABLE:
                record_success("T0")
            return result
        except Exception as e:
            print(f"[T0] ERROR (attempt {attempt+1}/{MAX_RETRY+1}): {e}")
            if attempt < MAX_RETRY:
                import time
                time.sleep(5 * (attempt + 1))  # Exponential backoff
            else:
                write_step_result("T0", {"error": str(e), "retries": MAX_RETRY})
                print(f"[T0] ❌ Failed after {MAX_RETRY+1} attempts")
                if WATCHDOG_AVAILABLE:
                    record_failure("T0", str(e))
...
def run_step_T1():
    ...
    for attempt in range(MAX_RETRY + 1):
        try:
            ...
            print(f"[T1] ✅ Success: {result['total_stocks']} stocks, {result['new_candidates']} candidates")
            if WATCHDOG_AVAILABLE:
                record_success("T1")
            return result
        except Exception as e:
            print(f"[T1] ERROR (attempt {attempt+1}/{MAX_RETRY+1}): {e}")
            if attempt < MAX_RETRY:
                import time
                time.sleep(5 * (attempt + 1))
            else:
                write_step_result("T1", {"error": str(e), "retries": MAX_RETRY})
                print(f"[T1] ❌ Failed after {MAX_RETRY+1} attempts")
                if WATCHDOG_AVAILABLE:
                    record_failure("T1", str(e))
                return {"error": str(e), "retries": MAX_RETRY}
...
def run_step_T3():
    ...
    for attempt in range(MAX_RETRY + 1):
        try:
            ...
            print(f"[T3] ✅ Success: {result['candidates']} candidates")
            if WATCHDOG_AVAILABLE:
                record_success("T3")
            return result
        except Exception as e:
            print(f"[T3] ERROR (attempt {attempt+1}/{MAX_RETRY+1}): {e}")
            if attempt < MAX_RETRY:
                import time
                time.sleep(5 * (attempt + 1))
            else:
                write_step_result("T3", {"error": str(e), "retries": MAX_RETRY})
                print(f"[T3] ❌ Failed after {MAX_RETRY+1} attempts")
                if WATCHDOG_AVAILABLE:
                    record_failure("T3", str(e))
                return {"error": str(e), "retries": MAX_RETRY}
...
def run_step_T4():
    ...
    for attempt in range(MAX_RETRY + 1):
        try:
            ...
            print(f"[T4] ✅ Success: {result['final_candidates']} candidates")
            if WATCHDOG_AVAILABLE:
                record_success("T4")
            return result
        except Exception as e:
            print(f"[T4] ERROR (attempt {attempt+1}/{MAX_RETRY+1}): {e}")
            if attempt < MAX_RETRY:
                import time
                time.sleep(5 * (attempt + 1))
            else:
                write_step_result("T4", {"error": str(e), "retries": MAX_RETRY})
                print(f"[T4] ❌ Failed after {MAX_RETRY+1} attempts")
                if WATCHDOG_AVAILABLE:
                    record_failure("T4", str(e))
                return {"error": str(e), "retries": MAX_RETRY}
...
def run_step_T5():
    ...
    for attempt in range(MAX_RETRY + 1):
        try:
            ...
            print(f"[T5] ✅ Success: {len(signals)} signals")
            if WATCHDOG_AVAILABLE:
                record_success("T5")
            return result
        except Exception as e:
            print(f"[T5] ERROR (attempt {attempt+1}/{MAX_RETRY+1}): {e}")
            if attempt < MAX_RETRY:
                import time
                time.sleep(5 * (attempt + 1))
            else:
                write_step_result("T5", {"error": str(e), "retries": MAX_RETRY})
                print(f"[T5] ❌ Failed after {MAX_RETRY+1} attempts")
                if WATCHDOG_AVAILABLE:
                    record_failure("T5", str(e))
                return {"error": str(e), "retries": MAX_RETRY}
...
def run_step_NIGHT():
    ...
    for attempt in range(MAX_RETRY + 1):
        try:
            ...
            print(f"[NIGHT] ✅ Success: {len(results['steps_ran'])} steps summarized")
            if WATCHDOG_AVAILABLE:
                record_success("NIGHT")
            return results

        except Exception as e:
            print(f"[NIGHT] ERROR (attempt {attempt+1}/{MAX_RETRY+1}): {e}")
            if attempt < MAX_RETRY:
                time.sleep(5 * (attempt + 1))  # Exponential backoff
            else:
                write_step_result("NIGHT", {"error": str(e), "retries": MAX_RETRY})
                print(f"[NIGHT] ❌ Failed after {MAX_RETRY+1} attempts")
                if WATCHDOG_AVAILABLE:
                    record_failure("NIGHT", str(e))
                return {"error": str(e), "retries": MAX_RETRY}
...
def run_step_BATTLE():
    ...
    for attempt in range(MAX_RETRY + 1):
        try:
            ...
            print(f"[BATTLE] ✅ Success: {len(plan['watchlist'])} watchlist items")
            if WATCHDOG_AVAILABLE:
                record_success("BATTLE")
            return plan

        except Exception as e:
            print(f"[BATTLE] ERROR (attempt {attempt+1}/{MAX_RETRY+1}): {e}")
            if attempt < MAX_RETRY:
                time.sleep(5 * (attempt + 1))  # Exponential backoff
            else:
                write_step_result("BATTLE", {"error": str(e), "retries": MAX_RETRY})
                print(f"[BATTLE] ❌ Failed after {MAX_RETRY+1} attempts")
                if WATCHDOG_AVAILABLE:
                    record_failure("BATTLE", str(e))
                return {"error": str(e), "retries": MAX_RETRY}
...
def run_step_GUARDIAN():
    ...
    # Watchdog check (integrated with Guardian)
    if WATCHDOG_AVAILABLE:
        try:
            watchdog_result = run_watchdog_check()
            health["watchdog"] = watchdog_result
            print(f"[GUARDIAN] Watchdog: {watchdog_result.get('status')}")
        except Exception as wd_e:
            print(f"[GUARDIAN] Watchdog check failed: {wd_e}")
            health["watchdog"] = {"status": "error", "error": str(wd_e)}
...
if __name__ == "__main__":
    ...
    ap.add_argument("--step", required=False, choices=list(STEP_HANDLERS.keys()), help="Pipeline step to execute")
    ap.add_argument("--full-day", action="store_true", help="Run full trading day pipeline (T0→T1→T3→T4→T5)")
    ap.add_argument("--watchdog", action="store_true", help="Run watchdog check independently")
    args = ap.parse_args()
    ...
    # Watchdog mode
    if args.watchdog:
        print("[WATCHDOG] Independent watchdog check mode...")
        if WATCHDOG_AVAILABLE:
            result = run_watchdog_check()
            print(f"\n[WATCHDOG] Result: {json.dumps(result, ensure_ascii=False, default=str)}")
        else:
            print("[WATCHDOG] ❌ Watchdog module not available")
            sys.exit(1)
        sys.exit(0)

    # Step mode
    if not args.step:
        print("[ERROR] --step is required when not using --watchdog")
        sys.exit(1)
